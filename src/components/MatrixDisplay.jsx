...
const grid = Array.from({ length: 3 }, (_, row) =>
  Array.from({ length: 3 }, (_, col) => {
    const yIndex = 2 - row // yIndex 2=top, 0=bottom
  ...
  return (
    <div className="w-full overflow-x-auto">
      <div className="min-w-[280px]">
        {/* X-axis labels */}
        <div className="grid grid-cols-[auto_1fr_1fr_1fr] mb-1">
          <div className="w-6 md:w-8" />
          {X_LABELS.map(label => (
            <div key={label} className="text-center text-xs text-muted-foreground px-1 font-medium">
              {label}
            </div>
          ))}
        </div>
        ...
        {grid.map((row, rowIndex) => (
          <div key={rowIndex} className="grid grid-cols-[auto_1fr_1fr_1fr] gap-1 mb-1">
            {/* Y-axis label */}
